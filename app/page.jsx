...
/**
 * Poster Builder – Mapness‑style Layout
 * Single-file React page (drop in /app/page.tsx for Next.js 13/14 App Router)
 *
 * - Top stepper like Mapness
 * - Left: step panels (prompt, text, size, material/frame)
 * - Right: sticky poster preview with frame mock
 * - Bottom sticky order bar (price + CTA)
 * - Uses your existing /api/generate-image endpoint (returns { imageUrl })
 *
 * TailwindCSS assumed.
 */
  ...
  return (
    <section className="bg-gradient-to-b from-white to-neutral-50 border-b">
      <div className="mx-auto max-w-6xl px-4 py-10 sm:py-14">
        ...
        <p className="mt-2 text-neutral-600 max-w-2xl">Voer je idee in, bekijk direct een voorbeeld, voeg een persoonlijke tekst toe en bestel in een paar klikken.</p>
      </div>
    </section>
  );
}
...
type SizeOption = { id: string; label: string; widthMm: number; heightMm: number; basePrice: number };

const SIZE_OPTIONS: Record<ShapeId, SizeOption[]> = {
  square: [
...
type MaterialId = "paper" | "canvas";
const MATERIALS: { id: MaterialId; label: string; multiplier: number }[] = [
...
function Builder() {
  ...
  const [sizeId, setSizeId] = useState<string>(SIZE_OPTIONS.square[0].id);
  const [material, setMaterial] = useState<MaterialId>("paper");
  const [frame, setFrame] = useState<FrameId>("noframe");
  ...
  async function onGenerate() {
    ...
    try {
      ...
      setImageUrl(`https://picsum.photos/${Math.max(pxDims.width,512)}/${Math.max(pxDims.height,512)}?random=${encodeURIComponent(prompt)}`);
    } finally {
  ...
  async function exportPNG() {
    ...
    try {
      ...
      canvas.width = pxDims.width; canvas.height = pxDims.height;
      ...
      if (overlayText.trim()) drawMultilineCentered(ctx, overlayText.trim(), canvas.width/2, canvas.height - margin, textBoxWidth, fontSize * 1.2);
      const url = canvas.toDataURL("image/png");
      const a = document.createElement("a"); a.href = url; a.download = `poster-${size.label.replace(/\s+/g, "_")}.png`; a.click();
    ...
    <section className="mx-auto max-w-6xl px-4 py-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div className="space-y-6">
        <Panel title="1. Afbeelding genereren" desc="Schrijf je idee, klik daarna op ‘Genereer’.">
          <textarea value={prompt} onChange={(e)=>setPrompt(e.target.value)} placeholder="Bijv. Minimalistische skyline bij zonsondergang, warme pastelkleuren" className="w-full min-h-[96px] rounded-xl border px-3 py-2 focus:outline-none focus:ring-2 focus:ring-black/10" />
          <div className="mt-3 flex gap-3">
            <Button onClick={onGenerate} disabled={isGenerating || !prompt.trim()}>{isGenerating?"Bezig…":"Genereer afbeelding"}</Button>
            <Button variant="ghost" onClick={()=>setImageUrl(`https://picsum.photos/${Math.max(pxDims.width,512)}/${Math.max(pxDims.height,512)}?random=${Date.now()}`)}>Test met voorbeeld</Button>
          </div>
        </Panel>

        <Panel title="2. Persoonlijke tekst" desc="Komt vast onderaan, mooi gecentreerd.">
          <input value={overlayText} onChange={(e)=>setOverlayText(e.target.value)} type="text" placeholder="Bijv. Voor oma – 2025" className="w-full rounded-xl border px-3 py-2 focus:outline-none focus:ring-2 focus:ring-black/10" />
        </Panel>
        ...
        <Panel title="3. Formaat & materiaal" desc="Kies vorm, afmeting, materiaal en lijst.">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Select label="Materiaal" value={material} onChange={(v)=>setMaterial(v as MaterialId)} options={MATERIALS.map(m=>({value:m.id,label:m.label}))} />
            <Select label="Lijst" value={frame} onChange={(v)=>setFrame(v as FrameId)} options={FRAME_OPTIONS.map(f=>({value:f.id,label:`${f.label}${f.extra?` (+€ ${f.extra})`:''}`}))} />
          </div>
        </Panel>
      ...
      {/* Right column: sticky preview like Mapness */}
      <div className="lg:sticky lg:top-20 self-start">
        <div className="rounded-3xl bg-white shadow-sm ring-1 ring-black/5 overflow-hidden">
          <div className="border-b p-4"><div className="font-semibold">Voorbeeld</div><div className="text-xs text-neutral-500">Live weergave van je poster</div></div>
          <div className="p-4">
            <div className="w-full bg-neutral-100 rounded-2xl overflow-hidden border relative" style={{aspectRatio: shape==="square"?"1 / 1":`${size.widthMm} / ${size.heightMm}`}}>
              {imageUrl?(
              ...
              {overlayText && (
                <div className="absolute left-1/2 -translate-x-1/2 bottom-6 w-[80%] text-center text-neutral-900/90">
                  <div className="line-clamp-3 text-ellipsis break-words font-medium" style={{fontSize: shape==="square"?"clamp(14px,2.8vw,24px)":"clamp(14px,2.4vw,24px)"}}>{overlayText}</div>
                </div>
              )}
...
function BottomBar() {
  return (
    <div className="sticky bottom-0 z-40 border-t bg-white/90 backdrop-blur supports-[backdrop-filter]:bg-white/70">
...
function Pill({ label, active, onClick }: { label: string; active?: boolean; onClick?: () => void }) {
  return <button onClick={onClick} className={`rounded-full px-4 py-2 text-sm border transition ${active?"bg-black text-white border-black":"bg-white hover:bg-neutral-50 border-neutral-200"}`}>{label}</button>;
}
function OptionTile({ children, active, onClick }: { children: React.ReactNode; active?: boolean; onClick?: () => void }) {
  return <button onClick={onClick} className={`rounded-xl border p-3 text-left transition ${active?"border-black ring-1 ring-black":"border-neutral-200 hover:border-neutral-300"}`}>{children}</button>;
}
...
function mmToPixels(wMm: number, hMm: number, ppi = 300) {
  const inchesW = wMm / 25.4; const inchesH = hMm / 25.4;
...
function drawCoverImage(ctx: CanvasRenderingContext2D, img: HTMLImageElement, cw: number, ch: number) {
  const ir = img.width / img.height, cr = cw / ch; let dw = cw, dh = ch;
  if (ir > cr) { dh = ch; dw = Math.round(dh * ir); } else { dw = cw; dh = Math.round(dw / ir); }
  const dx = Math.round((cw - dw) / 2), dy = Math.round((ch - dh) / 2); ctx.drawImage(img, dx, dy, dw, dh);
}
function drawMultilineCentered(ctx: CanvasRenderingContext2D, text: string, centerX: number, baselineY: number, maxWidth: number, lineHeight: number) {
  ...
  if (line) lines.push(line); const total = lines.length * lineHeight; let y = baselineY - total;
  for (const l of lines) { ctx.fillText(l, centerX, y + lineHeight); y += lineHeight; }
}
